...
.grid-left { flex: 1; text-align: left; font-size: 11px; line-height: 1.5; }
.grid-center { width: 130px; text-align: center; font-weight: bold; color: #f39c12; font-size: 11px; line-height: 1.4; border-left: 1px solid #2d3442; border-right: 1px solid #2d3442; padding: 0 4px; }
.grid-right { flex: 1; text-align: left; font-size: 11px; line-height: 1.5; padding-left: 12px; }
...
if has_dhan_api and dhan_token != "eyJ0eX...token_code_yahan":
...
# ==============================================================================
# 4. मुख्य ऑप्शन चेन टेबल ग्रिड (RESPONSIVE FOR MOBILE WINDOWS)
# ==============================================================================
...
# ==============================================================================
# 5. 2.4 & 2.5 QUANTUM COLONIES (COMPACT RESPONSIVE FLEX WITH TOTAL OI & VOL PCR)
# ==============================================================================
st.markdown("---")
...
<div class="grid-3-col">
    <div class="grid-left">
        ...
        <span class="txt-red">OI PCR: {otm_oi} | ChgOI: {otm_choi}</span><br>
        <span style="color: #a3b8cc;">VOL PCR: {otm_vol} | ChgVOL: {otm_chg_vol}</span>
    </div>
    <div class="grid-center">
        <span style="color:#ffffff; font-size:12px;">ATM {atm_strike_base}</span><br>
        <div style="background-color:#2c1f11; padding:2px; border-radius:3px; margin-top:3px; color:#f39c12; font-size:10px;">OI TOT PCR: {tot_oi_pcr_24}</div>
        <div style="background-color:#11222c; padding:2px; border-radius:3px; margin-top:2px; color:#3498db; font-size:10px;">VOL TOT PCR: {tot_vol_pcr_24}</div>
    </div>
    <div class="grid-right">
        <span style='color: #3498db; font-weight: bold;'>🔵 ITM CLUSTER (Right)</span><br>
        <span class="txt-blue">OI PCR: {itm_oi} | ChgOI: {itm_choi}</span><br>
        <span style="color: #a3b8cc;">VOL PCR: {itm_vol} | ChgVOL: {itm_chg_vol}</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div class="grid-3-col">
    <div class="grid-left">
        ...
        <span style="color: #f39c12;">OI: {plus5_cross_oi} | ChgOI: {plus5_cross_choi}</span><br>
        <span style="color: #a3b8cc;">VOL: {plus5_cross_vol} | ChgVOL: {plus5_cross_chgvol}</span>
    </div>
    <div class="grid-center">
        <span style="color:#ffffff; font-size:12px;">ATM {atm_strike_base}</span><br>
        <div style="background-color:#2c1f11; padding:2px; border-radius:3px; margin-top:3px; color:#f39c12; font-size:10px;">OI TOT PCR: {tot_oi_pcr_25}</div>
        <div style="background-color:#11222c; padding:2px; border-radius:3px; margin-top:2px; color:#3498db; font-size:10px;">VOL TOT PCR: {tot_vol_pcr_25}</div>
    </div>
    <div class="grid-right">
        <span style='color: #9b59b6; font-weight: bold;'>⬇️ -5 CORRIDOR (Right)</span><br>
        <span style="color: #9b59b6;">OI: {minus5_cross_oi} | ChgOI: {minus5_cross_choi}</span><br>
        <span style="color: #a3b8cc;">VOL: {minus5_cross_vol} | ChgVOL: {minus5_cross_chgvol}</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
# ==============================================================================
# 7. EXPIRY DAY SPECIAL AI CONFIDENCE ENGINE (LOGIC & FORMULAS LOCKED)
# ==============================================================================
st.markdown("---")
...
mock_low_boundary = atm_strike_base - (2 * interval)
mock_high_boundary = atm_strike_base + (2 * interval)
mock_liq_point = atm_strike_base - interval if not is_market_green else atm_strike_base + interval

st.markdown(f"""
<div style="background-color: #161b22; padding: 12px; border-radius: 6px; border: 1px solid #3498db; font-size: 12px; line-height: 1.6;">
    <span style="color: #3498db; font-weight: bold; font-size: 13px;">🔄 14-RADAR LIVE REVERSAL VALIDATION SENSORS:</span><br><br>
    • <b>🎯 Previous 50 Candle Swing High Ceiling:</b> Strike <span class="txt-red"><b>{atm_strike_base + interval}</b></span> (Max Liquidity Compression Resistance Barrier Wall)<br>
    • <b>🎯 Previous 50 Candle Swing Low Floor:</b> Strike <span class="txt-green"><b>{atm_strike_base - interval}</b></span> (Heavy Call Unwinding Absorption Reversal Zone)<br>
    • <b>💎 High Liquidity Cluster Point:</b> Strike <b>{mock_liq_point}</b> (Order Matching Block - Operators Trapping Core)<br>
    • <b>📊 Previous Day High (PDH) Corridor:</b> Strike <span class="txt-red"><b>{atm_strike_base + (3 * interval)}</b></span> Barrier Floor<br>
    • <b>📊 Previous Day Low (PDL) Corridor:</b> Strike <span class="txt-green"><b>{atm_strike_base - (3 * interval)}</b></span> Floor Support<br>
    • <b>⏳ Weekly OHLC Equilibrium Range:</b> Strike <b>{atm_strike_base - interval}</b> to <b>{atm_strike_base + interval}</b> Matrix<br>
    • <b>🛑 OPERATOR TRAP ZONE LIMIT:</b> Strike <b>{atm_strike_base}</b> (High Volatility Churning Box - Avoid Premium Leakage)
</div>
""", unsafe_allow_html=True)
...
if is_40pt_frictional_range:
    st.markdown(f"""
    <div style="background-color: #2c1a1d; padding: 12px; border-radius: 6px; border: 1px solid #e67e22; font-size: 12px;">
        <span style="color: #e67e22; font-weight: bold; font-size: 13px;">⚠️ 25-CANDLE CRITICAL CHOPPY ALERT (40-POINTS COMPRESSION ACTIVE)</span><br>
        Market structure index pichle 25 candle se exact strike price <b>{atm_strike_base}</b> ke andar tight lock compression consolidation mein fasa hai. 
        Data Matrix ke mutabik <span class="txt-green"><b>Explosive Gamma Breakout Seek</b></span> generate ho raha hai. Range burst hote hi premium multiplication alert trigger hoga!
    </div>
    """, unsafe_allow_html=True)
