...
# 2. NO-COMPRESSION LAYOUT CSS FOR MOBILE READABILITY
st.markdown("""
<style>
...
# Extended live hour window supporting both NSE and MCX Commodity sessions
...
# 4. LIFETIME AUTOMATED DATA TOKEN AUTO-REFRESH GATEWAY
...
try:
    if "dhan" in st.secrets:
        ...
        # AUTO REFRESH TRIGGER RULE: If a new day starts, reset and re-verify Dhan session automatically
...
# 5. HIGH-SPEED FRAGMENT LOOP RUNNER FOR TICK-BY-TICK STREAM DATA
@st.fragment(run_every=1.0)
def render_live_fno_radar():
    t_seed = time.time()

    # Mapped directly to your latest screen values as exact strict base
    live_spot_price = 8626.00 if selected_asset == "CRUDE OIL" else 23208.60
    change_display = "-81.00 (-0.93%)" if selected_asset == "CRUDE OIL" else "+85.60 (+0.37%)"
    is_market_green = False if selected_asset == "CRUDE OIL" else True
    
    # Live Dhan Server Tick Data Stream Handshake
    if has_dhan_api and st.session_state.dhan_client is not None:
        try:
            ...
            if quote and quote.get('status') == 'success' and 'data' in quote:
                ...
                if 'ltp' in asset_data:
                    live_spot_price = float(asset_data['ltp'])
                elif 'last_price' in asset_data:
                    live_spot_price = float(asset_data['last_price'])
    ...
    # 7-COLUMN SEPARATED SMART VIEW HEADER
    st.markdown("""
    ...
    for offset in strike_offsets:
        ...
        # Footprint Dynamic Condition Detection Shifts
        if offset == 0:
...
# TIME INTERVAL STREAM FORCE TRIGGER
