...
import numpy as np
import datetime
import pytz  # भारतीय समय क्षेत्र सिंक लॉक करने के लिए
...
# ⏱️ लाइव डेटा टाइम वॉच क्लॉक फिक्स (सटीक भारतीय समय - IST Lock)
...
# कलर ग्रेडिंग के लिए थ्रेशोल्ड निर्धारित करने हेतु रैंडम वॉल्यूम वेक्टर्स जनरेट करना
...
for idx, s in enumerate(strikes):
    ...
    # 🕵️ 1. टाइम-स्टैम्प और अनवाइंडिंग ट्रैकर लॉजिक (कब से कितने मिनट से फेज शुरू हुआ)
    ...
    # 🎨 कड़े कंडीशनल रो कलर असाइनमेंट नियम
    if idx == highest_idx:
        row_css = "class='row-highest-green'"
    elif idx == highest_pe_idx:
        row_css = "class='row-highest-red'"
    elif idx == next_75_idx:
        row_css = "class='row-next-yellow'"
    else:
...
# --- 🧠 ऑल-इन-वन वेटेड AI डिसीजन स्कोर कैलकुलेटर Engine ---
...
# --- 🚀 एक्सपायरी डे स्पेशल AI कॉन्फिडेंस इंजन (IV + Theta Sync) ---
...
html += '<span ' + green_dot_style + '>●🚀 QUANTUM FUSION METRICS ALERT: 🟢 TAKE CALL BUY ACTIVE (🎯 Confidence: ' + str(ai_call_score) + '%)'html += '<span ' + red_dot_style + '>●⚠️ REVERSAL SATARK ZONE & OHLC LEVELS: • Pull-Back @ ' + pbl + '-' + pbh + ' | • Pull-Down @ ' + pdl + '-' + pdh + ''html += ''st.markdown(html, unsafe_allow_html=True)लाइव वेब ऑडियो अलार्म (40% वॉल्यूम)st.markdown("document.getElementById('algoAlarm').volume = 0.4;", unsafe_allow_html=True)
