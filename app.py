...
st.write("• " + f"{atm_strike_base - 100}" + " - " + f"{atm_strike_base + 100}" + " ke dayre mein bade operators puri tarah surakshit rehkar theta decay vasulenge.")
st.write("")
...
with col_rev1:
    st.success("🔄 Pull-Back Support Range (Put Side 🟢): " + f"{atm_strike_base - 150} - {atm_strike_base - 100}")
    st.write(f"Logic: Max(50C Swing Low, PDL, PDC) = {calc_support_floor} | Heavy put writing aur absorption zone")
with col_rev2:
    st.error("🛑 Pull-Down Resistance Wall (Call Side 🔴): " + f"{atm_strike_base + 100} - {atm_strike_base + 150}")
