...
st.write("📲 5-कॉलम मोबाइल फ़िट | लेफ्ट ओआई - राइट वॉल्यूम लेआउट")
...
def get_broker_ui_data():
    ...
    for s in strikes:
        ...
        pe_chg_pct = np.random.uniform(-25, -2) if s in [23550, 23600] else np.random.uniform(2, 45)
        ...
        # आपकी डिमांड के अनुसार: स्ट्राइक के लेफ्ट में OI डेटा, राइट में VOL डेटा सेट किया गया
        rows.append({
            "CE Phase": ce_phase,
            "OI / (Chg OI)\n(Strike Left)": f"{ce_label}\n({ce_chg_sign})",
            "ST/Strike\nPCR | ChgPCR": f"🟡 ATM {s}\n{oi_pcr:.2f}\n({chg_oi_pcr:.2f})" if s == 23350 else f"{s}\n{oi_pcr:.2f}\n({chg_oi_pcr:.2f})",
            "VOLUME / (VolPCR)\n(Strike Right)": f"P: {pe_vol:.1f}k ({vol_pcr:.2f})\nC: {ce_vol:.1f}k ({1/vol_pcr:.2f})",
            "PE Phase": pe_phase
        })
