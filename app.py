...
st.set_page_config(layout="wide", page_title="Quant Decision Engine")

# सीएसएस स्टाइलिंग: ग्रिड लुक को और अधिक प्रोफेशनल बनाने के लिए
st.markdown("<style>.reportview-container{background:#06080c;}th{background-color:#111827!important;color:#9ca3af!important;text-align:center!important;}td{text-align:center!important;font-family:monospace;padding:5px!important;}div[data-testid='stMetricValue']{font-size:20px!important;font-family:monospace;}</style>", unsafe_allow_html=True)
st.title("📊 क्वांटिटेटिव SMC + VSA + Greeks इंस्टीट्यूशनल डिसीजन इंजन")
st.write("स्ट्राइक प्राइस सेंटर लेआउट | प्रोप्राइटरी मल्टी-लेयर क्वांटम परतें | रियल-टाइम रिस्क मैनेजर")
...
def get_quantum_data():
    ...
    for s in strikes:
        ...
        bb_str = f"{bb_raw:.1f}% " + ("(SQUEEZE)" if bb_raw < 2.5 else "(EXPAND)")
        gamma_raw = np.random.randint(86, 99) if s == 23450 else np.random.randint(12, 60)
        
        alert = "Structure"
        if s == 23450 and gamma_raw > 85: alert = "💥 GAMMA BLAST"
        elif s == 23250: alert = "🚨 Put OB Active"
        ...
        rows.append({
            ...
            "Target Pro%": "65%" if s == 23350 else f"{np.random.randint(41, 55)}%", "BB Squeeze": bb_str, "Gamma Str%": f"{gamma_raw}%", "SMC Alerts": alert, "Strike PCR": f"{strike_pcr:.2f}",
            "strike_num": s, "ce_raw_oi": ce_oi, "pe_raw_oi": pe_oi, "ce_raw_chg": ce_chg, "pe_raw_chg": pe_chg, "ce_raw_vol": ce_vol, "pe_raw_vol": pe_vol
        })
...
is_gamma = any("💥 GAMMA BLAST" in str(x) for x in df['SMC Alerts'])
is_trap = float(df[df['strike_num'] == 23350]['CE ChgOI'].iloc[0]) > 35.0
...
# 3. मुख्य ग्रिड डिस्प्ले
st.markdown("---")
st.subheader("📋 प्रोप्राइटरी हाइब्रिड डेटा मैट्रिक्स")
display_cols = ["CE Phase", "CE Vol PCR", "CE ChgOI", "CE OI", "ST/Strike", "PE OI", "PE ChgOI", "PE Vol PCR", "PE Phase", "Target Pro%", "BB Squeeze", "Gamma Str%", "SMC Alerts", "Strike PCR"]
