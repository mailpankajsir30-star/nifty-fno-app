...
st.markdown("""
    ...
    .cell-phase { width: 22%; font-size: 11px; line-height: 1.4; text-align: left; padding-left: 5px; }
    .cell-data { width: 22%; font-size: 11px; line-height: 1.4; text-align: left; padding-left: 5px; }
...
# 🔄 AUTOMATIC DAY CHECKER (Monday vs Tuesday Expiry Sync)
# Live calendar check: Agar week-day index 1 (Tuesday) hai ya kal expiry hai toh automatic detect karega
current_weekday = ist_now.weekday() 

if current_weekday == 1: # 1 matlab Tuesday (Asli Expiry Day)
...
# 📈 REAL BROKER-MATCH PRICE ENGINE (LOCKED ON 23218.30)
base_nifty = 23218.30
t_seed = time.time()
live_oscillation = math.sin(t_seed) * 1.45
...
# ==============================================================================
# 4. मुख्य ऑप्शन चेन टेबल ग्रिड (OI & CHANGE OF OI REALISTIC SCALE)
# ==============================================================================
...
for i in range(-5, 6):
    ...
    # Real Broker Terminal Weight Simulation Logic
    if strike_num == 23100:
        ce_oi_str, pe_oi_str = "28.32L", "1.98Cr"
        ce_chg_str, pe_chg_str = "+773.93%", "+421.95%"
    elif strike_num == 23150:
        ce_oi_str, pe_oi_str = "31.95L", "1.15Cr"
        ce_chg_str, pe_chg_str = "+1538.12%", "+453.20%"
    elif strike_num == 23200:
        ce_oi_str, pe_oi_str = "88.27L", "2.24Cr"
        ce_chg_str, pe_chg_str = "+514.57%", "+374.61%"
    elif strike_num == 23250:
        ce_oi_str, pe_oi_str = "66.13L", "85.37L"
        ce_chg_str, pe_chg_str = "+974.25%", "+304.91%"
    elif strike_num == 23300:
        ce_oi_str, pe_oi_str = "1.27Cr", "95.46L"
        ce_chg_str, pe_chg_str = "+184.41%", "+29.16%"
    elif strike_num == 23350:
        ce_oi_str, pe_oi_str = "57.01L", "41.25L"
        ce_chg_str, pe_chg_str = "+166.00%", "+61.17%"
    else:
        computed_lakhs = round(45.4 + abs(i) * 12.5 + (sec_tick * 0.1), 2)
        ce_oi_str, pe_oi_str = f"{computed_lakhs}L", f"{computed_lakhs - 5}L"
        ce_chg_str, pe_chg_str = f"+{round(150 + sec_tick * 2, 2)}%", f"+{round(90 + sec_tick, 2)}%"
    ...
    if i == 0:
        ...
        oi_details = f"<b>{ce_oi_str}</b> ({ce_chg_str})<br>{live_pcr:.2f}<br>{round(144.6 + sec_tick, 1)}k"
        vol_details = f"{dynamic_vol:.1f}L / 12.1%<br>{dynamic_chg_vol}k"
    elif i == 5:
        strike_label = f"<b>{strike_num}</b>"
        pcr_label = f"{round(live_pcr * 0.35, 2)}"
        ...
        oi_details = f"<span class='txt-red'>🔴</span> <b>{ce_oi_str}</b> ({ce_chg_str})<br>{round(live_pcr*0.35, 2)}"
        ...
        oi_details = f"<b>{ce_oi_str}</b> ({ce_chg_str})<br>1.10"
