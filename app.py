...
# सीएसएस: डार्क लुक को बनाए रखने और पैडिंग को बारीक करने के लिए
st.markdown("""
    <style>
...
st.write("📲 6-कॉलम परफेक्ट मोबाइल फिट | 100% सेफ़ नेटिव लेआउट")
...
def get_broker_ui_data():
    ...
    for s in strikes:
        ...
        pe_chg_pct = np.random.uniform(-25, -2) if s in [23350, 23400] else np.random.uniform(2, 45)
        ...
        # \n का उपयोग करके डेटाफ्रेम के अंदर ही कड़े तरीके से वर्टिकल अलाइनमेंट लॉक कर दिया
        rows.append({
            "CE Phase": ce_phase,
            "CE Stats\nOI (Chg) | Vol (PCR)": f"{ce_label} ({ce_chg_sign})\n{ce_vol:.1f}k ({1/vol_pcr:.2f})",
            "ST/Strike\nPCR | ChgPCR": f"🟡 ATM {s}\n{oi_pcr:.2f}\n({chg_oi_pcr:.2f})" if s == 23350 else f"{s}\n{oi_pcr:.2f}\n({chg_oi_pcr:.2f})",
            "PE Stats\nOI (Chg) | Vol (PCR)": f"{pe_label} ({pe_chg_sign})\n{pe_vol:.1f}k ({vol_pcr:.2f})",
            "PE Phase": pe_phase
        })
...
# 3. मुख्य ग्रिड डिस्प्ले (प्योर सेफ़ st.dataframe)
st.markdown("---")
st.subheader("📋 प्रोप्राइटरी हाइब्रिड डेटा मैट्रिक्स")

# कलर कोडिंग स्टाइल रूल्स
...
# यूआई पर पूरी ग्रिड टेबल को रेंडर करना
st.dataframe(df.style.map(color_master_ui), use_container_width=True, hide_index=True)
