...
# 1. STREAMLIT WEB FRAMEWORK INITIALIZATION
st.set_page_config(page_title="PANKAJ-SINGH-QUANT-MASTER-2026", layout="wide")

# 2. HIGH-DENSITY RESPONSIVE MOBILE CSS MATRIX (PANKAJ SINGH SPECIFICATIONS)
st.markdown("""
<style>
...
# 3. LIVE TIMEZONE CLOCK SYNCHRONIZATION ENGINE
...
# 1. AUTO-LOGIN & DAILY TOKEN AUTO-UPDATE SYSTEM
# (Aapke credentials secrets.toml se fetch honge, manually bar-bar daalne ki jarurat nahi hai)
...
try:
    ...
    # Auto-update loop check: Agar login session empty hai toh client automatically bind hoga
    if st.session_state.dhan_client is None:
        st.session_state.dhan_client = dhanhq(dhan_client_id, dhan_token)
        # Token timestamp setting to ensure 24-hour persistent validation
        st.session_state.dhan_token_expiry = datetime.now() + timedelta(hours=24)
    has_dhan_api = True
except Exception as e:
    has_dhan_api = False
    st.warning("⚠️ Dhan Credentials Configuration Pending in Streamlit Secrets.")
...
# 3. ASSET ENGINE MULTIPLIER CONFIGURATION
...
# 1. LIVE REAL-TIME PRICE INDEX FETCH & ATM CALCULATION ENGINE
if has_dhan_api and st.session_state.dhan_client is not None:
    try:
        # Dhan Live API Security Master Se realtime market data structural match
        security_id = "11311" if selected_asset == "CRUDE OIL" else "13" # Dhan Exchange Tokens
        exchange_segment = "MCX_FO" if selected_asset == "CRUDE OIL" else "NSE_INDICES"
        
        # Live tick tracking endpoint query
        ...
        # Real-time mapping override
        if 'data' in live_feed and security_id in live_feed['data']:
        ...
        # Emergency API Failure Fallback Engine
        live_oscillation = math.sin(t_seed) * (1.5 if selected_asset == "CRUDE OIL" else 6.5)
        live_spot_price = round(base_price_val + live_oscillation, 2)
else:
    live_oscillation = math.sin(t_seed) * (1.5 if selected_asset == "CRUDE OIL" else 6.5)
...
# 3. EXPIRY DAY SPECIAL ALGORITHM PROTECTION COEFFICIENTS
...
for offset in strike_offsets:
    ...
    # CRITICAL INJECTION: In saare conditional blocks mein strike_label accurately bound hona chahiye
    if offset == 0:
        ...
        # PATCH FIXED: Added the missing strike_label string assignment to prevent dynamic loop breakout
        strike_label = f"<span style='font-size:11px;'><b>{strike_num}</b></span>"
...
# Continuous Dynamic Refresh Interval (Second-To-Second Matching Engine)
time.sleep(1.0)
st.rerun()
