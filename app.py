...
# 📈 LIVE COMPUTE MATRIX ENGINE (LOCKED ON REAL BEARISH 23231.50)
base_nifty = 23231.50
t_seed = time.time()
live_oscillation = math.sin(t_seed) * 2.15
...
# ==============================================================================
# 4. मुख्य ऑप्शन चेन टेबल ग्रिड (OI & CHANGE OF OI LIVE DYNAMIC ENERGY)
# ==============================================================================
...
<div class="master-row-container" style="background-color: #1f242d; border-bottom: 2px solid #2d3442; font-weight: bold;">
    ...
    <div class="cell-data" style="text-align:center;">OI Details<br>(VOL / OI PCR)<br>(Chg OI / Chg PCR)</div>
...
for i in range(-5, 6):
    ...
    base_oi = 44.4 + abs(i) * 3.2
    chg_oi_factor = round(base_oi * (1 + math.sin(t_seed + i) * 0.05), 2)
    dynamic_oi_pct = round(10.5 + (sec_tick * 0.1), 1)
    ...
    if i == 0:
        ...
        oi_details = f"{chg_oi_factor:.1f}L (+{dynamic_oi_pct}%)<br>{live_pcr:.2f}<br>{round(144.6 + sec_tick, 1)}k / 3.15"
        vol_details = f"{dynamic_vol:.1f}L / 12.1%<br>{dynamic_chg_vol}k<br>0.35"
    elif i == 5:
        ...
        oi_details = f"<span class='txt-red'>🔴</span> {chg_oi_factor+40:.1f}L (+12.2%)<br>{round(live_pcr*0.35, 2)}<br>{round(73.2 + sec_tick, 1)}k"
        vol_details = f"{dynamic_vol+25:.1f}L / 5.2%<br>{dynamic_chg_vol+30}k"
    elif i == -5:
        strike_label = f"<b>{strike_num}</b>"
        pcr_label = f"{round(live_pcr * 2.1, 2)}"
        ...
        oi_details = f"{chg_oi_factor-5:.1f}L (+2.1%)<br>{round(live_pcr*2.1, 2)}<br>{round(106.3 + sec_tick, 1)}k"
        vol_details = f"<span class='txt-green'>🟢</span> {dynamic_vol+15:.1f}L / 8.3%<br>{dynamic_chg_vol+80}k"
    else:
        ...
        oi_details = f"{chg_oi_factor:.1f}L (+{dynamic_oi_pct}%)<br>1.10<br>{dynamic_chg_vol:.1f}k / 0.95"
        vol_details = f"{dynamic_vol:.1f}L / 5.1%<br>{dynamic_chg_vol}k<br>1.05"
...
# ==============================================================================
# 🔄 NATIVE LOOP UPDATE BACKEND SYNC (Bina stop hue lagatar ghumega)
# ==============================================================================
time.sleep(1.0)
st.rerun()
