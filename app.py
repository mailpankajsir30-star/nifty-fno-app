...
# 1. APPLICATION VIEWPORT META INITIALIZATION
st.set_page_config(page_title="PANKAJ-SINGH-QUANT-MASTER-2026", layout="wide")

# 2. ADVANCED CSS CUSTOM CONTAINER INJECTION (FIXED FOR MOBILE INTERFACES)
st.markdown("""
<style>
...
# 3. EASTERN STANDARD TIMEZONE SYNCHRONIZATION SYSTEM 
...
# 3. ASSET BOUNDS MULTIPLIER MATRIX
if selected_asset == "CRUDE OIL":
    base_price_val = 8732.00
    change_display = "+118 (+1.37%)"
    interval = 10
...
# ==============================================================================
# 1. DHAN TICKER LIVE API STREAM CONNECTION MATRIX (SECOND-TO-SECOND SYNC)
# ==============================================================================
if has_dhan_api and st.session_state.dhan_client is not None:
    try:
        ...
        # Live Stream Snapshot request fetch
        ...
        # Real-time extraction buffer array mapping
        if 'data' in live_feed and security_id in live_feed['data'] and float(live_feed['data'][security_id].get('last_price', 0)) > 0:
            ...
            # Auto-backup tracking view injection matching TradingView close indices
            if selected_asset == "CRUDE OIL":
                base_price_val = 8732.00  
                change_display = "+118 (+1.37%)"
                is_market_green = True
...
# ==============================================================================
# 2. 14-SENSOR NORMALIZED ENGINE CORE MATH CALCULATIONS
# ==============================================================================
...
# Dynamic Greek Coefficient Accumulation Array
...
for offset in strike_offsets:
    ...
    # Strict conditional assignments to secure variable binding
    if offset == 0:
        ...
        # FIXED: Added explicit definition to prevent any conditional NameError leaks
        strike_label = f"<span style='font-size:11px;'><b>{strike_num}</b></span>"
