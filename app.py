...
# 1. DIRECT KEY BINDING ENGINE (Permanently destroys secrets settings warning)
if 'dhan_client' not in st.session_state:
    st.session_state.dhan_client = None
    st.session_state.dhan_token_expiry = None

try:
    # ⚠️ UPDATE LOG: Apni actual numeric ID aur alpha-numeric token " " ke andar paste kijiye
    dhan_client_id = "B7G59C82" 
    dhan_token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJyZWdpc3RyYXRpb25JZCI6IjEyMDUzNzcwMzciLCJjbGllbnRJZCI6IjExMDIyMzM5MTEiLCJleHAiOjE3ODExMjYwMDB9"
...
# 1. LIVE REAL-TIME PRICE INDEX FETCH VIA DHANHQ SDK CLIENT
...
# 2. 14-SENSOR DATA COEFFICIENTS DEPLOYMENT
...
for offset in strike_offsets:
    ...
    if offset == 0:
        ...
        # FIXED PROPER: Added definition to destroy NameError loops
        strike_label = f"<span style='font-size:11px;'><b>{strike_num}</b></span>"
...
# 9. HIGH FREQUENCY REFRESH HEARTBEAT CONTROLLER
time.sleep(1.0)
st.rerun()
