...
import math
import time
import requests  # Required for Google Live Bypass
...
# 2. FIXED CELL SPACING CSS FOR MOBILE VISIBILITY (NO COMPRESSION)
st.markdown("""
<style>
...
# 4. ANGEL ONE AUTOMATED ENCRYPTED LOGIN GATEWAY
...
try:
    if "angel" in st.secrets:
        angel_api_key = st.secrets["angel"]["api_key"]
        angel_client_id = st.secrets["angel"]["client_id"]
        angel_password = st.secrets["angel"]["password"]
        angel_totp_secret = st.secrets["angel"]["totp_secret"]

        if st.session_state.angel_client is None:
            smart_conn = SmartConnect(api_key=angel_api_key)
            totp = pyotp.TOTP(angel_totp_secret)
            current_totp = totp.now()
            
            session_data = smart_conn.generateSession(angel_client_id, angel_password, current_totp)
            if session_data and session_data.get('status'):
                st.session_state.angel_client = smart_conn
                has_angel_api = True
        else:
            has_angel_api = True
except Exception:
...
# 5. HIGH-SPEED TICK STREAM ENGINE
t_seed = time.time()

# Absolute live price sync checkpoints
...
# STEP A: Try Fetching from Angel API
if has_angel_api and st.session_state.angel_client is not None:
    try:
        token_id = "705294" if selected_asset == "CRUDE OIL" else "99926000"
        exchange_segment = "MCX" if selected_asset == "CRUDE OIL" else "NSE"
        trading_symbol = "CRUDEOIL" if selected_asset == "CRUDE OIL" else "NIFTY"
        live_feed = st.session_state.angel_client.ltpData(exchange_segment, trading_symbol, token_id)
        
        if live_feed and live_feed.get('status') and live_feed.get('data') is not None:
            live_spot_price = float(live_feed['data']['ltp'])
            close_price = float(live_feed['data'].get('close', live_spot_price))
            net_change = live_spot_price - close_price
            pct_change = (net_change / close_price) * 100 if close_price > 0 else 0.0
            change_display = f"{'+' if net_change >= 0 else ''}{net_change:.2f} ({pct_change:.2f}%)"
            is_market_green = net_change >= 0
            has_fallback_active = False
        else:
            has_fallback_active = True
    except Exception:
        has_fallback_active = True
else:
    has_fallback_active = True

# STEP B: 🚀 GOOGLE FINANCE LIVE BYPASS (If Angel Fails, Fetch real numbers instantly!)
if has_fallback_active:
    try:
        ticker = "MCX:CRUDEOIL1!" if selected_asset == "CRUDE OIL" else "NSE:NIFTY"
        url = f"https://yahoo.com{ticker.split(':')[-1]}.NS" if selected_asset == "NIFTY 50" else f"https://yahoo.comCL=F"
        headers = {'User-Agent': 'Mozilla/5.0'}
        res = requests.get(url, headers=headers, timeout=2)
        if res.status_code == 200:
            json_data = res.json()
            meta = json_data['chart']['result'][0]['meta']
            live_spot_price = float(meta['regularMarketPrice'])
            prev_close = float(meta['previousClose'])
            net_change = live_spot_price - prev_close
...
for offset in strike_offsets:
    ...
    if selected_asset == "NIFTY 50":
        if offset == 0:  # ATM CENTER 
            ...
            ce_oi_str = f"{round(3.39 + offset*0.2, 2)} Cr"
            ce_chg_str = f"+{round(1.06 + dynamic_multiplier, 2)} Cr"
            pe_oi_str = f"{round(1.78 - offset*0.3, 2)} Cr" if offset < 0 else f"{round(98.8 - offset*10, 1)} L"
            pe_chg_str = f"+{int(63 + dynamic_multiplier*10)} L"
            strike_pcr = f"{round(0.29 + offset*0.1, 2)}"
        ...
        if offset == 0:
            ce_oi_str, ce_chg_str, pe_oi_str, pe_chg_str = "15.4L", "+110%", "42.1L", "+195%"
            strike_pcr, cal_ob, put_ob = "2.73", "<span class='txt-purple'>🐳 BIG PLAYER LONG</span>", "<span class='txt-red'>🚨 INSTITUTION WRITING</span>"
            ce_phase, pe_phase = "<span class='txt-green'>⚠️ SMART BUY</span>", "<span class='txt-blue'>⚠️ SMART SELL</span>"
        else:
            ...
            cal_ob, put_ob = "<span style='color:#888;'>⏳ Waiting (S.M)</span>", "<span style='color:#888;'>⏳ Waiting (S.M)</span>"
            ce_phase = "Call Writing" if offset > 0 else "Long Unwinding"
            pe_phase = "Put Writing" if offset < 0 else "Short Build-up"
    ...
    # 🟡 FIXED ATM COLOR AND LABEL LOGIC FOR ALL OFFSETS
    if offset == 0:
        strike_display = f"<div><span class='txt-yellow' style='font-size:13px;'><b>ATM</b></span><br><span class='txt-yellow'><b>{strike_num}</b></span><br><span style='color:#fff; font-size:11px;'>PCR:{strike_pcr}</span></div>"
    else:
...
# CLASSIC RELOAD ENGINE FOR INTER-SECOND LOOPS
if is_market_hours:
    time.sleep(1.0)
    st.rerun()
