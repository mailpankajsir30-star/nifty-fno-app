...
for i in range(-5, 6):
    ...
    if i == 0:
        # ATM पर स्मार्ट मनी फुटप्रिंट मिल चुके हैं (FSS: 86) -> इसलिए 'Waiting Zone (SM)' गायब (Hide) हो गया है
        strike_label = f"<span class='txt-yellow'>🟡 ATM {strike_num}</span>"
        pcr_label = "1.37<br>(3.15)"
        ce_phase = "<span class='txt-green'>⚠️ SMART MONEY ACTIVE<br>Long Build-up (86+)</span><br><span class='txt-yellow'>🟡</span><br>11:30 AM"
        pe_phase = "<span class='txt-blue'>⚠️ SMART MONEY ACTIVE<br>Short Covering (84+)</span><br><span class='txt-yellow'>🟡</span><br>09:45 AM"
        oi_details = "21.7L (+31.3%)<br>2.85<br>144.6k / 3.15"
        vol_details = "29.8L / 12.1%<br>50.7k<br>0.35"
    elif i == 5:
        # रेजिस्टेंस बैरियर पर भी स्मार्ट मनी फुटप्रिंट पुष्ट (FSS: 90) -> 'Waiting Zone (SM)' हाइड हो गया है
        strike_label = f"<b>{strike_num}</b>"
        pcr_label = "0.50<br>(2.41)"
        ce_phase = "<span class='txt-purple'>💣 INSTITUTIONAL ATTACK<br>Short Covering (90+)</span><br><span class='txt-yellow'>🟡</span><br>10:15 AM"
        pe_phase = f"{pe_waiting_prefix}<span class='txt-red'>Short Buildup (83+)</span><br>01:10 PM"
        oi_details = "<span class='txt-red'>🔴</span> 104.0L (+12.2%)<br>0.72<br>73.2k / 2.41"
        vol_details = "89.9L / 5.2%<br>115.1k<br>1.58"
    elif i == -5:
        # सपोर्ट बैरियर सिंक -> 'Waiting Zone (SM)' हाइड है
        strike_label = f"<b>{strike_num}</b>"
        pcr_label = "3.29<br>(0.63)"
        ce_phase = f"{ce_waiting_prefix}<span class='txt-blue'>Short Covering (85+)</span><br>10:15 AM"
        pe_phase = "<span class='txt-purple'>💣 INSTITUTIONAL ATTACK<br>Heavy Put Writing (79+)</span><br>01:10 PM"
        oi_details = "14.5L (+2.1%)<br>0.51<br>106.3k / 0.63"
        vol_details = "<span class='txt-green'>🟢</span> 47.6L / 8.3%<br>210.2k<br>1.98"
    else:
        # सामान्य कतारें: यहाँ अभी कोई स्मार्ट मनी फुटप्रिंट नहीं है, इसलिए 'Waiting Zone (SM)' दिख रहा है,
        # लेकिन उसके नीचे कॉल राइटिंग/पुट राइटिंग की सेलर्स एक्टिविटी लगातार लाइव कैच होकर बिना वेट किए दिख रही है!
        strike_label = f"{strike_num}"
        pcr_label = f"1.{abs(i)}5<br>(1.12)"
        ce_phase = f"{ce_waiting_prefix}<span class='txt-red'>Call Writing Active ({45 + abs(i)}+)</span><br>09:15 AM"
        pe_phase = f"{pe_waiting_prefix}<span class='txt-green'>Put Writing Active ({42 + abs(i)}+)</span><br>09:15 AM"
...
# ==============================================================================
# 7. EXPIRY DAY SPECIAL AI CONFIDENCE ENGINE (सख्त संडे/क्लोज्ड मार्केट फ़िल्टर)
# ==============================================================================
st.markdown("---")
st.markdown("### 🍏 4. EXPIRY DAY SPECIAL AI CONFIDENCE ENGINE (IV & THETA SYNC)")

is_market_open_now = False  # बाजार सत्र वीकेंड पर बंद रहता है
