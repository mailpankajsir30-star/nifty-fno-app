...
st.markdown("""
    ...
    /* 🎨 रो का गंदा बैकग्राउंड हमेशा के लिए डिलीट, अब केवल एटीएम रो हाइलाइट रहेगी */
    .row-atm-blue { background-color: #172554 !important; font-weight: bold !important; }
    
    /* 🟢🔴 Death/Premium बिंदी सिग्नल्स जो केवल फ़ेज़ टेक्स्ट के साथ दिखेंगे */
    .dot-highest-green { color: #22c55e; font-size: 14px; font-weight: bold; display: inline-block; margin-right: 4px; }
    .dot-highest-red { color: #ef4444; font-size: 14px; font-weight: bold; display: inline-block; margin-right: 4px; }
    .dot-next-yellow { color: #eab308; font-size: 14px; font-weight: bold; display: inline-block; margin-right: 4px; }
    ...
    /* 🟣🟢🔴 नीचे डिसीजन पैनल की बड़ी बिंदी ब्लिंकिंग */
...
# ⏱️ लाइव डेटा टाइम वॉच क्लॉक फिक्स (भारतीय मानक समय - IST Lock)
...
for idx, s in enumerate(strikes):
    ...
    # 🎨 बिंदी लॉजिक - टेक्स्ट के आगे कौन सा इंडिकेटर सिंबल जुडेगा
    ce_dot = "<span class='dot-highest-green'>● </span>" if idx == highest_idx else ("<span class='dot-next-yellow'>● </span>" if idx == next_75_idx else "")
    pe_dot = "<span class='dot-highest-red'>● </span>" if idx == highest_pe_idx else ""
    
    row_css = "class='row-atm-blue'" if s == current_atm else ""
...
for r in rows_data:
    ...
    html += '<tr ' + r['css'] + '><td style="color:' + cpc + '; font-weight:bold; font-size:10px; text-align:center;">' + r['ce_dot'] + r['cph'] + '<span class="time-lbl">' + r['t_ce'] + '</span></td>'
    ...
    html += '<td style="color:' + ppc + '; font-weight:bold; font-size:10px; text-align:center;">' + r['pe_dot'] + r['pph'] + '<span class="time-lbl">' + r['t_pe'] + '</span></td></tr>'
